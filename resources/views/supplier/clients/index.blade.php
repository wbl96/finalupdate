...
@section('content')
    <div class="card">
        <div class="card-body">
            <div class="mb-3">
                <button class="btn btn-import btn-sm" data-bs-toggle="modal" data-bs-target="#infoModal">
            ...
            <table class="table display table-striped table-bordered" id="clientsTable" style="width:100%">
                <thead>
                    <tr>
                        <th>#</th>
                        <th class="text-center">{{ trans('users.store name') }}</th>
                        <th class="text-center">{{ trans('global.mobile') }}</th>
                        <th class="text-center">{{ trans('global.email') }}</th>
                        <th class="text-center">{{ trans('orders.purchases num') }}</th>
                    </tr>
                </thead>
            </table>
        </div>
    ...
    <div id="editClientModalContainer"></div>

    {{-- include create modal --}}
    @include('supplier.clients.create_modal')
@endsection
...
@push('script')
    ...
    <script>
        ...
        var table = $('#clientsTable').DataTable({
            ...
            serverSide: true,
            responsive: true,
            ajax: "{!! route('supplier.clients.getClients') !!}",
            columns: [{
                data: 'DT_RowIndex',
                searchable: false
            }, {
        ...
        function showEdit(url) {
            $("#editClientModalContainer").empty()
            ...
            $.ajax({
                ...
                success: function(result) {
                    $("#editClientModalContainer").html(result);
