            ...
            <table class="table display table-striped table-bordered" id="ordersTable" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th scope="col">{{ __('products.name.name') }}</th>
                        <th scope="col">{{ __('products.details.details') }}</th>
                        <th class="text-center">{{ trans('orders.Price per piece') }}</th>
                        <th class="text-center">{{ trans('orders.Quantity') }}</th>
...
@push('script')
    ...
    <script>
        ...
        var table = $('#ordersTable').DataTable({
            ...
            serverSide: true,
            responsive: true,
            ajax: "{!! route('supplier.orders.get-orders') !!}",
            columns: [{
                data: 'order_num',
                name: 'order_num'
            }, {
