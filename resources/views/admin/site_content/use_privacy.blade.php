...
@section('title', trans('site_content.use & privacy policy'))
...
@section('content')
    ...
    <div class="container mt-4">
        <h5 class="mb-3">{{ trans('site_content.use & privacy policy') }}</h5>
        <form action="{{ route('admin.site-content.terms.update', [$policy]) }}" method="POST" id="policyForm">
            @csrf
            @method('put')
            <div id="editor-container" style="height: 500px;"></div>
            <textarea name="content" id="policyContent" class="d-none">{!! old('content', $policyContent) ?? null !!}</textarea>
            {{-- Save Button --}}
