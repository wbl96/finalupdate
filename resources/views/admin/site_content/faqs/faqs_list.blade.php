...
@section('title', trans('site_content.faqs'))
...
@section('content')
    <div class="card">
        <div class="card-body">
            <div class="mb-3">
                ...
                <button class="add-service-btn btn btn-import btn-sm" data-bs-toggle="modal" data-bs-target="#addNewFaqModal">
            ...
            <table class="table display table-striped table-bordered" id="servicesTable" style="width:100%">
                <thead>
                    <tr>
                        <th>#</th>
                        <th class="text-center">{{ trans('site_content.question ar') }}</th>
                        <th class="text-center">{{ trans('site_content.answer ar') }}</th>
    ...
    <div id="editFaqModalContainer"></div>
    @include('admin.site_content.faqs.create_faq_modal')
@endsection
...
@push('script')
    ...
    <script>
        ...
        var table = $('#servicesTable').DataTable({
            ...
            serverSide: true,
            responsive: true,
            ajax: "{!! route('admin.site-content.faq.getFaqs') !!}",
            columns: [{
                data: 'DT_RowIndex',
                searchable: false
            }, {
        ...
        function _delete(id, name) {
            // url
            let url = `{{ route('admin.site-content.faq.destroy', [':TARGET']) }}`;
        ...
        function showEdit(url) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    $("#editFaqModalContainer").html(result);
                    $('#modal_freeze').on('hidden.bs.modal', function() {
                        $('#editFaqModal').modal('show').addClass('show')
                    });
                    setTimeout(() => {
