...
@section('title', trans('categories.the categories'))
...
@section('content')
    <div class="card">
        <div class="card-body">
            ...
            {{-- table --}}
            <table class="table display table-striped table-bordered" id="categoriesTable" style="width:100%">
                <thead>
                    <tr>
                        <th>#</th>
                        <th class="text-center">{{ trans('categories.name.ar') }}</th>
                        <th class="text-center">{{ trans('categories.name.en') }}</th>
                        <th class="text-center">{{ trans('categories.#sub categories') }}</th>
    ...
    <div id="editCategoriyModalContainer"></div>
...
@push('script')
    ...
    <script>
        ...
        function _delete(id, name) {
            // url
            let url = `{{ route('admin.categories.destroy', [':TARGET']) }}`;
        ...
        function showEdit(url) {
            $("#editCategoriyModalContainer").empty()
            ...
            $.ajax({
                ...
                success: function(result) {
                    $("#editCategoriyModalContainer").html(result);
                    $('#modal_freeze').on('hidden.bs.modal', function() {
                        $('#editCategoryModal').modal('show').addClass('show')
                    });
                    setTimeout(() => {
